%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is a modified version of the thesis.tex file to easily enable
% me to create a PDF for a single chapter so I can check and edit it
% easily
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\RequirePackage[l2tabu]{nag}	
\documentclass[a4paper,12pt,leqno,openbib,oldfontcommands]{memoir}
\input{latex-configuration}

% The following \marginparwidth is to keep the todonotes package happy. It can be revised once the todo's are done and the package removed e.g. to help make the thesis more attractive in PDF format.
\setlength{\marginparwidth}{2cm}

\begin{document}
% Read https://www.overleaf.com/learn/latex/How_to_Write_a_Thesis_in_LaTeX_(Part_1):_Basic_Structure

\pagenumbering{roman}

\renewcommand{\contentsname}{Table of Contents}
\maxtocdepth{subsection}
\tableofcontents*
\addtocontents{toc}{\par\nobreak \mbox{}\hfill{\bf Page}\par\nobreak}
%\clearemptydoublepage
%
\listoftables
\addtocontents{lot}{\par\nobreak\textbf{{\scshape Table} \hfill Page}\par\nobreak}
%\clearemptydoublepage
%
\listoffigures
\addtocontents{lof}{\par\nobreak\textbf{{\scshape Figure} \hfill Page}\par\nobreak}
\clearemptydoublepage

% https://tex.stackexchange.com/a/99656/5125
\renewcommand{\listoflistings}{%
    \cleardoublepage
    \phantomsection
    \addcontentsline{toc}{chapter}{Sourcecode Listings}%
    \listof{listing}{Sourcecode Listings}%
}
\listoflistings\clearpage

\listofprerequisites\clearpage



%
%
% The bulk of the document is delegated to these chapter files in
% subdirectories.
\mainmatter
%
\input{frontmatter/abstract.tex}


\begin{comment}
\setcounter{chapter}{-1}
\chapter{The Red Thread! (not part of the final thesis)}
\input{meta-chapters/the-red-thread}


\input{mainchapters/introduction}
\end{comment}
\input{mainchapters/related-work}
\begin{comment}
\input{mainchapters/a-grounding-in-the-domain}
\end{comment}
% Now using Marian's version of the methodology
\input{mainchapters/methodology-for-marian}

\begin{comment}

\input{mainchapters/case-studies-overview}
\end{comment}
\input{mainchapters/analytics-in-use}
\input{mainchapters/apps-and-their-artefacts}
\input{mainchapters/tools-and-their-artefacts}

\chapter{Discussion [exhibition temporarily closed]}
The old discussion chapter is offline pending extensive renovations to reflect the new orientation of the findings.

Estimated date for reopening: \sout{\nth{1} February 2022} \nth{13} July 2022. Draft available in Google Docs  for my collaborators using their Google accounts.

{\small\href{https://docs.google.com/document/d/1tdmyTwoFuxMYJNMBw7yWxmpRDVeOuQ4yReNKQKcq9GM/edit?usp=sharing}{docs.google.com/document/d/1tdmyTwoFuxMYJNMBw7yWxmpRDVeOuQ4yReNKQKcq9GM/edit?usp=sharing}}

The old discussion follows:

\input{mainchapters/discussion}


\begin{comment}
%\input{mainchapters/a-grounding-in-the-domain}
%\input{mainchapters/evaluation}
\clearpage

%\chapter{Catrobat Case Study}
\input{empirical-studies/case-studies}
\input{empirical-studies/analytics-tools}
\input{mainchapters/future-work}
\end{comment}

\backmatter
\bibliographystyle{ACM-Reference-Format}
\refstepcounter{chapter}
\bibliography{./references/bibliography_for_core_thesis,./empirical-studies/catrobat-case-study,./references/bibliography_on_phd_experience,./references/logging_paper.bib}
%%%% TODO Consider having separate sections e.g. for books and/or a separate bibliography for AppendixB.

% \clearemptydoublepage
\appendix
\input{appendicies/thematic-analysis}
%\input{appendicies/software-contributions}
%\input{empirical-studies/data-sources}
%\input{case-studies/analytics-tools}
%\input{appendicies/our-android-vitals-tools}
%\input{appendicies/on-mobile-analytics}
%\input{appendicies/learning-R}
%\input{meta-chapters/actions.tex}
%\input{meta-chapters/possible-architectures}
%\input{appendicies/phd-experience}
% Add index
%\printindex
%   
\end{document}