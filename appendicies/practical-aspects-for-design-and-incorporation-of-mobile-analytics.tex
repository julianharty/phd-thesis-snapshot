...
The reliability of software where the outcomes of failure are material has been a subject of discussion and research for decades. As \citep{dobbing1998reliability} notes, where the reliability requirements are modest black box testing techniques may be sufficient, however \emph{``When reliability claims cannot be justified from test results alone, safety standards accept evidence from the design process"}. This paper focuses on smart instrumentation for the UK nuclear industry, nonetheless given the widespread use and implicit trust of analytics software, similar approaches to assess the reliability of this software could help in terms of auditing the behaviours of the analytics tools. Indeed two of the authors of \citep{dobbing1998reliability} collaborated with a third author and published a paper on the relevance and importance of software in measuring systems, where \emph{``Both users and suppliers of such systems must be aware of the risks involved and take appropriate precautions."}~\citep{wichmann2007software}.
...
These two terms, verification and validation, are often used in tandem, particularly in software testing standards including the retired~\citep{BS_7925_1_1998} and the standard that superseded it~\citep{iso29119-1-2013}. The definitions from the ISO standard are:
\begin{itemize}
    \item ``Verification is confirmation, through the provision of objective evidence, that specified requirements have been fulfilled in a given work item."~\citep{iso29119-1-2013}
    \item ``Validation demonstrates that the work item can be used by the users for their specific tasks."~\citep{iso29119-1-2013}
\end{itemize}
...
As the requirements for analytics tools are often proprietary, verification using the product specified requirements may be impractical to assess rigorously unless and until one has access to these requirements. Nonetheless common-sense requirements can be established based on heuristics and experience, \emph{etc}. this is covered in the section titled: \href{sec:rubric-for-evaluating-analytics-tools}{\nameref{sec:rubric-for-evaluating-analytics-tools}}.
...
Testing of the analytics and testing of the incorporation of analytics into an app are both relevant. In terms of testing the incorporation, there is a request to add a fake implementation of one of the mature and popular mobile analytics libraries~\citep{segmentio_analytics_android_issue_770_fake_implementation_for_testing}. One of the core developers acknowledges such a feature does not exist and it may be useful. 
...
\subsection{A rubric for evaluating analytics tools}~\label{sec:rubric-for-evaluating-analytics-tools}
